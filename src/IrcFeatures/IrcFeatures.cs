...
namespace Meebey.SmartIrc4net
{
    ...
    public class IrcFeatures : IrcClient
    {
        ...
        {
            get { return _ExternalIpAdress; }
            set { _ExternalIpAdress = value; }
        }
        ...
        {
            get { return new ReadOnlyCollection<DccConnection>(_DccConnections); }
        }
        ...
        private readonly Dictionary<string, CtcpDelegate> _CtcpDelegates =
            new Dictionary<string, CtcpDelegate>(StringComparer.CurrentCultureIgnoreCase);
        ...
        public IrcFeatures()
        {
            ...
            _CtcpDelegates.Add("version", CtcpVersionDelegate);
            _CtcpDelegates.Add("clientinfo", CtcpClientInfoDelegate);
            _CtcpDelegates.Add("time", CtcpTimeDelegate);
            _CtcpDelegates.Add("userinfo", CtcpUserInfoDelegate);
            _CtcpDelegates.Add("url", CtcpUrlDelegate);
            _CtcpDelegates.Add("source", CtcpSourceDelegate);
            _CtcpDelegates.Add("finger", CtcpFingerDelegate);
            // The DCC Handler
            _CtcpDelegates.Add("dcc", CtcpDccDelegate);
            // Don't remove the Ping handler without your own implementation
            _CtcpDelegates.Add("ping", CtcpPingDelegate);
        }
        ...
        {
            var chat = new DccChat(this, user, _ExternalIpAdress, passive, priority);
        ...
        public void SendFile(string user, string filepath)
        {
            ...
            if (fi.Exists)
            {
                SendFile(user, new FileStream(filepath, FileMode.Open), fi.Name, fi.Length, DccSpeed.RfcSendAhead, false,
                         Priority.Medium);
            }
        }
        ...
        public void SendFile(string user, string filepath, bool passive)
        {
            ...
            if (fi.Exists)
            {
                SendFile(user, new FileStream(filepath, FileMode.Open), fi.Name, fi.Length, DccSpeed.RfcSendAhead,
                         passive, Priority.Medium);
            }
        }
        ...
        public void SendFile(string user, Stream file, string filename, long filesize, DccSpeed speed, bool passive,
                             Priority priority)
        {
            var send = new DccSend(this, user, _ExternalIpAdress, file, filename, filesize, speed, passive, priority);
        ...
        {
            SendMessage(SendType.CtcpReply, e.Data.Nick,
                        "VERSION " + ((CtcpVersion == null) ? VersionString : CtcpVersion));
        }

        private void CtcpClientInfoDelegate(CtcpEventArgs e)
        {
            string clientInfo = "CLIENTINFO";
            foreach (var kvp in _CtcpDelegates)
            {
        ...
        private void CtcpPingDelegate(CtcpEventArgs e)
        {
            ...
            {
                SendMessage(SendType.CtcpReply, e.Data.Nick,
                            "PING " + e.Data.Message.Substring(6, (e.Data.Message.Length - 7)));
            }
            else
            {
        ...
        private void CtcpRfcPingDelegate(CtcpEventArgs e)
        {
            ...
            {
                SendMessage(SendType.CtcpReply, e.Data.Nick,
                            "PONG " + e.Data.Message.Substring(6, (e.Data.Message.Length - 7)));
            }
            else
            {
        ...
        {
            SendMessage(SendType.CtcpReply, e.Data.Nick,
                        "USERINFO " + ((CtcpUserInfo == null) ? "No user info given." : CtcpUserInfo));
        }

        private void CtcpUrlDelegate(CtcpEventArgs e)
        {
            SendMessage(SendType.CtcpReply, e.Data.Nick,
                        "URL " + ((CtcpUrl == null) ? "http://www.google.com" : CtcpUrl));
        }

        private void CtcpSourceDelegate(CtcpEventArgs e)
        {
            SendMessage(SendType.CtcpReply, e.Data.Nick,
                        "SOURCE " + ((CtcpSource == null) ? "http://smartirc4net.meebey.net" : CtcpSource));
        }
        ...
        private void CtcpDccDelegate(CtcpEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    case "CHAT":
                        var chat = new DccChat(this, _ExternalIpAdress, e);
                        _DccConnections.Add(chat);
                        ThreadPool.QueueUserWorkItem(chat.InitWork);
                        break;
                    case "SEND":
                        ...
                        {
                            var send = new DccSend(this, _ExternalIpAdress, e);
                            _DccConnections.Add(send);
                            ThreadPool.QueueUserWorkItem(send.InitWork);
                        }
                        break;
